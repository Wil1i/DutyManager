...
const dbChecker = async () => {
  ...
  await Setting.findOne({ where: { name: "password" } }).then(
    async (result) => {
  ...
  await Setting.findOne({ where: { name: "startCodePersoneli" } }).then(
    async (result) => {
      if (!result || !result.value) {
        await Setting.create({
  ...
  await Setting.findOne({ where: { name: "lastCodePersoneli" } }).then(
    async (result) => {
      if (!result || !result.value) {
        await Setting.create({
  ...
  User.findAll().then(async (result) => {
    if (!result || !result[0]) {
      await axios
        ...
        .then((result) => {
          console.log(
            `Defualt user created [ ${result.id} | ${config.defualtUser.firstName} ${config.defualtUser.lastName} | ${config.defualtUser.userRank} ]`
          );
        });
    }
  });
